...
Removes the restriction on EVTio::linkauth for non-native actions named one of the five special action names:
...
Forward EVTio::setcode actions to the WebAssembly code deployed on the EVTio account.
*/
            {}
        })
...
const char*
builtin_protocol_feature_codename(builtin_protocol_feature_t codename) {
    auto itr = builtin_protocol_feature_codenames.find(codename);
    EVT_ASSERT(itr != builtin_protocol_feature_codenames.end(), protocol_feature_validation_exception,
...
protocol_feature_base::protocol_feature_base(protocol_feature_t                              feature_type,
    ...
    default: {
        EVT_THROW(protocol_feature_validation_exception,
...
void
builtin_protocol_feature::reflector_init() {
    protocol_feature_base::reflector_init();
...
digest_type
builtin_protocol_feature::digest() const {
...
fc::variant
protocol_feature::to_variant(bool                        include_subjective_restrictions,
                             fc::mutable_variant_object* additional_fields) const {
    EVT_ASSERT(builtin_feature, protocol_feature_exception, "not a builtin protocol feature");
...
protocol_feature_set::get_builtin_digest(builtin_protocol_feature_t feature_codename) const {
...
protocol_feature_set::get_protocol_feature(const digest_type& feature_digest) const {
    auto itr = _recognized_protocol_features.find(feature_digest);

    EVT_ASSERT(itr != _recognized_protocol_features.end(), protocol_feature_exception,
...
bool
protocol_feature_set::validate_dependencies(
...
const protocol_feature&
protocol_feature_set::add_feature(const builtin_protocol_feature& f) {
    auto builtin_itr = builtin_protocol_feature_codenames.find(f._codename);
    EVT_ASSERT(builtin_itr != builtin_protocol_feature_codenames.end(), protocol_feature_validation_exception,
    ...
    if(indx < _recognized_builtin_protocol_features.size()) {
        EVT_ASSERT(_recognized_builtin_protocol_features[indx] == _recognized_protocol_features.end(),
    ...
    for(const auto& d : f.dependencies) {
        ...
        EVT_ASSERT(itr != _recognized_protocol_features.end(), protocol_feature_exception,
                   ...
                   ("codename", f.builtin_feature_codename)("digest", feature_digest)("dependency_digest", d));
    ...
    if(expected_builtin_dependencies.size() > satisfied_builtin_dependencies.size()) {
        ...
        for(const auto& builtin_codename : missing_builtins) {
            ...
            EVT_ASSERT(itr != builtin_protocol_feature_codenames.end(),
                       protocol_feature_exception,
                       "Unexpected error");
    ...
    EVT_ASSERT(res.second, protocol_feature_exception,
...
void
protocol_feature_manager::init(chainbase::database& db) {
    EVT_ASSERT(!is_initialized(), protocol_feature_exception, "cannot initialize protocol_feature_manager twice");
...
const protocol_feature*
protocol_feature_manager::const_iterator::get_pointer() const {
    //EVT_ASSERT( _pfm, protocol_feature_iterator_exception, "cannot dereference singular iterator" );
    //EVT_ASSERT( _index != end_index, protocol_feature_iterator_exception, "cannot dereference end iterator" );
    return &*(_pfm->_activated_protocol_features[_index].iterator_to_protocol_feature);
}

uint32_t
protocol_feature_manager::const_iterator::activation_ordinal() const {
    EVT_ASSERT(_pfm,
               protocol_feature_iterator_exception,
               "called activation_ordinal() on singular iterator");
    EVT_ASSERT(_index != end_index,
               protocol_feature_iterator_exception,
...
uint32_t
protocol_feature_manager::const_iterator::activation_block_num() const {
    EVT_ASSERT(_pfm,
               protocol_feature_iterator_exception,
...
protocol_feature_manager::const_iterator::operator++() {
    EVT_ASSERT(_pfm, protocol_feature_iterator_exception, "cannot increment singular iterator");
    EVT_ASSERT(_index != end_index, protocol_feature_iterator_exception, "cannot increment end iterator");
...
protocol_feature_manager::const_iterator&
protocol_feature_manager::const_iterator::operator--() {
    EVT_ASSERT(_pfm, protocol_feature_iterator_exception, "cannot decrement singular iterator");
    if(_index == end_index) {
        EVT_ASSERT(_pfm->_activated_protocol_features.size() > 0,
                   protocol_feature_iterator_exception,
...
protocol_feature_manager::const_iterator
protocol_feature_manager::cbegin() const {
...
bool
protocol_feature_manager::is_builtin_activated(builtin_protocol_feature_t feature_codename,
...
void
protocol_feature_manager::activate_feature(const digest_type& feature_digest,
                                           uint32_t           current_block_num) {
    EVT_ASSERT(is_initialized(), protocol_feature_exception, "protocol_feature_manager is not yet initialized");

    auto itr = _protocol_feature_set.find(feature_digest);

    EVT_ASSERT(itr != _protocol_feature_set.end(), protocol_feature_exception,
               "unrecognized protocol feature digest: ${digest}", ("digest", feature_digest));

    if(_activated_protocol_features.size() > 0) {
        const auto& last = _activated_protocol_features.back();
        EVT_ASSERT(last.activation_block_num <= current_block_num,
                   protocol_feature_exception,
                   ...
                   ("current_block_num", current_block_num)("last_activation_block_num", last.activation_block_num));
    }

    EVT_ASSERT(itr->builtin_feature,
               protocol_feature_exception,
    ...
    EVT_ASSERT(indx < _builtin_protocol_features.size(), protocol_feature_exception,
               "invariant failure while trying to activate feature with digest '${digest}': "
               "unsupported builtin_protocol_feature_t ${codename}",
               ("digest", feature_digest)("codename", indx));

    EVT_ASSERT(_builtin_protocol_features[indx].activation_block_num == builtin_protocol_feature_entry::not_active,
               protocol_feature_exception,
...
void
protocol_feature_manager::popped_blocks_to(uint32_t block_num) {
    EVT_ASSERT(is_initialized(), protocol_feature_exception, "protocol_feature_manager is not yet initialized");
    ...
    while(_head_of_builtin_activation_list != builtin_protocol_feature_entry::no_previous) {
        ...
        if(e.activation_block_num <= block_num)
            break;
