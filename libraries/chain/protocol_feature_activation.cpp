/**
 *  @file
 *  @copyright defined in evt/LICENSE
 */
...
protocol_feature_activation::reflector_init() {
    ...
    EVT_ASSERT(protocol_features.size() > 0, ill_formed_protocol_feature_activation,
    ...
    for(const auto& d : protocol_features) {
        auto res = s.insert(d);
        EVT_ASSERT(res.second, ill_formed_protocol_feature_activation,
...
protocol_feature_activation_set::protocol_feature_activation_set(
    ...
    EVT_ASSERT(!enforce_disjoint || protocol_features.size() == expected_size,
