...
bool
first_preferred(const block_header_state& lhs, const block_header_state& rhs) {
...
void
fork_database::open(const std::function<void(block_timestamp_type,
                                             const flat_set<digest_type>&,
                                             const vector<digest_type>&)>& validator) {
    ...
    if(fc::exists(fork_db_dat)) {
        try {
            ...
            EVT_ASSERT(totem == magic_number, fork_database_exception,
            ...
            EVT_ASSERT(version >= min_supported_version && version <= max_supported_version,
            ...
            if(my->root->id == head_id) {
                ...
                EVT_ASSERT(my->head, fork_database_exception,
            ...
            if(candidate == my->index.get<by_lib_block_num>().end() || !(*candidate)->is_valid()) {
                EVT_ASSERT(my->head->id == my->root->id, fork_database_exception,
                           "head not set to root despite no better option available; '${filename}' is likely corrupted",
                           ("filename", fork_db_dat.generic_string()));
            }
            else {
                EVT_ASSERT(!first_preferred(**candidate, *my->head), fork_database_exception,
...
void
fork_database::reset(const block_header_state& root_bhs) {
    my->index.clear();
...
void
fork_database::rollback_head_to_root() {
...
void
fork_database::advance_root(const block_id_type& id) {
    EVT_ASSERT(my->root, fork_database_exception, "root not yet set");

    auto new_root = get_block(id);
    EVT_ASSERT(new_root, fork_database_exception,
               "cannot advance root to a block that does not exist in the fork database");
    EVT_ASSERT(new_root->is_valid(), fork_database_exception,
    ...
    for(auto b = new_root; b;) {
        ...
        EVT_ASSERT(b || blocks_to_remove.back() == my->root->id, fork_database_exception, "invariant violation: orphaned branch was present in forked database");
    }
...
fork_database::get_block_header(const block_id_type& id) const {
...
void
fork_database_impl::add(const block_state_ptr& n,
                        bool ignore_duplicate, bool validate,
                        const std::function<void(block_timestamp_type,
                                                 const flat_set<digest_type>&,
                                                 const vector<digest_type>&)>& validator) {
    EVT_ASSERT(root, fork_database_exception, "root not yet set");
    EVT_ASSERT(n, fork_database_exception, "attempt to add null block state");

    auto prev_bh = self.get_block_header(n->header.previous);

    EVT_ASSERT(prev_bh, unlinkable_block_exception,
    ...
    if(validate) {
        ...
        EVT_RETHROW_EXCEPTIONS(fork_database_exception, "serialized fork database is incompatible with configured protocol features")
    }
    ...
    if(!inserted.second) {
        if(ignore_duplicate)
            return;
        EVT_THROW(fork_database_exception, "duplicate block added", ("id", n->id));
    }
...
void
fork_database::add(const block_state_ptr& n, bool ignore_duplicate) {
...
fork_database::root() const {
    return my->root;
}

const block_state_ptr&
fork_database::head() const {
    return my->head;
}

block_state_ptr
fork_database::pending_head() const {
    const auto& indx = my->index.get<by_lib_block_num>();
...
branch_type
fork_database::fetch_branch(const block_id_type& h, uint32_t trim_after_block_num) const {
    branch_type result;
...
block_state_ptr
fork_database::search_on_branch(const block_id_type& h, uint32_t block_num) const {
    for(auto s = get_block(h); s; s = get_block(s->header.previous)) {
...
fork_database::fetch_branch_from(const block_id_type& first,
    ...
    EVT_ASSERT(first_branch, fork_db_block_not_found, "block ${id} does not exist", ("id", first));
    EVT_ASSERT(second_branch, fork_db_block_not_found, "block ${id} does not exist", ("id", second));
    ...
    while(first_branch->block_num > second_branch->block_num) {
        ...
        EVT_ASSERT(first_branch, fork_db_block_not_found,
    ...
    while(second_branch->block_num > first_branch->block_num) {
        ...
        EVT_ASSERT(second_branch, fork_db_block_not_found,
                   "block ${id} does not exist",
                   ("id", prev));
    }
...
void
fork_database::mark_valid(const block_state_ptr& h) {
    if(h->validated)
        return;
    ...
    EVT_ASSERT(itr != by_id_idx.end(), fork_database_exception,
...
block_state_ptr
fork_database::get_block(const block_id_type& id) const {
    auto itr = my->index.find(id);
    if(itr != my->index.end())
        return *itr;
