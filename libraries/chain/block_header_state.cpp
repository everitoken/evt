...
uint32_t
block_header_state::calc_dpos_last_irreversible(account_name producer_of_next_block) const {
...
block_header_state::next(block_timestamp_type when,
    ...
    if(itr != producer_to_last_produced.end()) {
        EVT_ASSERT(itr->second < (block_num + 1) - num_prev_blocks_to_confirm, producer_double_confirm,
    ...
    if(pending_schedule.schedule.producers.size() && result.dpos_irreversible_blocknum >= pending_schedule.schedule_lib_num) {
...
block_header_state
pending_block_header_state::_finish_next(
    const signed_block_header&                             h,
    const std::function<void(block_timestamp_type,
                             const flat_set<digest_type>&,
                             const vector<digest_type>&)>& validator) && {
    EVT_ASSERT(h.timestamp == timestamp, block_validate_exception, "timestamp mismatch");
    EVT_ASSERT(h.previous == previous, unlinkable_block_exception, "previous mismatch");
    EVT_ASSERT(h.confirmed == confirmed, block_validate_exception, "confirmed mismatch");
    EVT_ASSERT(h.producer == producer, wrong_producer, "wrong producer specified");
    EVT_ASSERT(h.schedule_version == active_schedule_version, producer_schedule_exception, "schedule_version in signed block is corrupted");
    ...
    if(h.new_producers) {
        ...
        EVT_ASSERT(h.new_producers->version == active_schedule.version + 1, producer_schedule_exception, "wrong producer schedule version specified");
        EVT_ASSERT(prev_pending_schedule.schedule.producers.size() == 0, producer_schedule_exception,
...
digest_type
block_header_state::sig_digest() const {
...
block_header_state::sign(const std::function<signature_type(const digest_type&)>& signer) {
    ...
    EVT_ASSERT(block_signing_key == fc::crypto::public_key(header.producer_signature, d),
